...
import {
  ...
  Star, Trash2, Check, CalendarRange, Table2, CalendarCheck, ListTodo, ChevronLeft, ChevronRight
} from 'lucide-react';
...
function MonthlyLimitsManager({ userId }: { userId: number }) {
  ...
  const { setMonthlyLimit, getAllMonthlyLimits, videoLimits } = useVideoLimits(userId);
  ...
  const fetchMonthlyLimits = useCallback(async () => {
    try {
      ...
      toast({
        title: "Error",
        description: "No se pudieron cargar los límites mensuales",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
  ...
  // Manejar la actualización de un límite mensual
  const handleUpdateLimit = async (year: number, month: number, newLimit: number, isProrated: boolean = false) => {
    try {
      const result = await setMonthlyLimit({
        ...
        month,
        isProrated,
        overrideReason: `Límite actualizado manualmente`
      });
      ...
      toast({
        title: "Error",
        description: "No se pudo actualizar el límite mensual",
        variant: "destructive"
      });
    }
  };
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {Array.from({ length: 12 }, (_, i) => i + 1).map(month => {
          const limit = monthlyLimits.find(l =>
            l.year === selectedYear && l.month === month
          );

          return (
            <Card key={month} className={cn(
              "border",
              limit ? "border-amber-200 bg-amber-50" : "border-gray-200"
            )}>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <Input
                      ...
                      min={1}
                      className="w-24"
                      defaultValue={limit?.maxVideos || videoLimits.monthlyLimit}
                      onChange={(e) => {
                        const newLimit = parseInt(e.target.value);
                        if (!isNaN(newLimit) && newLimit > 0) {
                          handleUpdateLimit(selectedYear, month, newLimit);
                        }
                      }}
                    />
