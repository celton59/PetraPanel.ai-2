...
export default function VideosPage() {
  ...
  // Add sorting state near the other state declarations
  ...
  const sortedAndFilteredVideos = useMemo(() => {
    ...
    if (sorting) {
      result.sort((a, b) => {
        const aVal = a[sorting.column as keyof typeof a];
        const bVal = b[sorting.column as keyof typeof b];
        ...
        // Handle dates
        if (sorting.column.toLowerCase().includes('date') || sorting.column.includes('At')) {
      ...
      <div className="space-y-3">
        {sortedAndFilteredVideos?.map((video) => (
          ...
          >
            ...
            {/* Thumbnail */}
            <div className="w-28 h-16 flex-shrink-0 overflow-hidden rounded mr3">
              <ThumbnailPreview
                src={video.thumbnailUrl}
                alt={video.title}
                aspectRatio="video"
      ...
      {(!videos || videos.length === 0)
        ...
        : (
          <>
            ...
            {pagination && sortedAndFilteredVideos.length > 0 && (
