...
export function useVideos() {
  ...
  const [updatingVideoId, setUpdatingVideoId] = useState<number>();
  ...
  const queryKey = ["/api/videos", page, limit, sort?.field, sort?.direction];
  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        const response = await api.get(queryKey[0] as string, { params });
  ...
  function getStatusLabel(role: string, video: ApiVideo) {
    switch (video.status) {
