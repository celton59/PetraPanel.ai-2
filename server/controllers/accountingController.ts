...
import { actionRates, payments, projects, userActions, users } from "@db/schema";
import { asc, eq, and, isNull, isNotNull, count, sql, desc } from "drizzle-orm";
import { db } from "@db";
import { type Express } from "express";


export async function getAccountingRates(req: Request, res: Response): Promise<Response> {
  try {
...
export async function createAccountingRate(req: Request, res: Response): Promise<Response> {
  try {
    // Verificar que el usuario sea administrador
    if (req.user?.role !== "admin") {
      return res.status(403).json({
...
export async function deleteAccountingRate(req: Request, res: Response): Promise<Response> {
  try {
    // Verificar que el usuario sea administrador
    if (req.user?.role !== "admin") {
      return res.status(403).json({
...
export async function getPendingPayments(req: Request, res: Response): Promise<Response> {
  try {
    // Verificar que el usuario sea administrador
    if (req.user?.role !== "admin") {
      return res.status(403).json({
...
export async function getUserAction(req: Request, res: Response): Promise<Response> {
  try {
...
export async function getPaymentHistory(req: Request, res: Response): Promise<Response> {
  try {
    // Verificar que el usuario sea administrador
    if (req.user?.role !== "admin") {
      return res.status(403).json({
...
export function setUpAccountingRoutes(
  ...
  app.post("/api/accounting/rates", requireAuth, createAccountingRate);
  ...
  app.get("/api/accounting/user-actions/:userId", getUserAction);
