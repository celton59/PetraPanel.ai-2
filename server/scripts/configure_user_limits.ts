...
async function configureUserLimits() {
  try {
    ...
    youtubers.forEach(user => {
      console.log(`${user.id} | ${user.username} | ${user.fullName} | ${user.maxAssignedVideos || 3}`);
    });
...
async function configureSpecificUser(rl: readline.Interface, youtubers: any[]) {
  rl.question("Ingresa el ID del usuario: ", (userId) => {
    ...
    rl.question(`Ingresa el nuevo límite para ${user.username} (actualmente: ${user.maxAssignedVideos || 3}): `, async (limit) => {
